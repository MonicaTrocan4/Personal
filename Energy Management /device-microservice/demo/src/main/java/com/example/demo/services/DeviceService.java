...
public class DeviceService {
    ...
    @Autowired
    public DeviceService(DeviceRepository deviceRepository, UserRepository userRepository, RabbitTemplate rabbitTemplate) {
    ...
    public List<DeviceDetailsDTO> findDeviceByName(String name) {
        ...
        if (devices.isEmpty()) {
            LOGGER.warn("No devices found with name: {}", name);
        }
    ...
    public UUID insert(DeviceDetailsDTO deviceDetailsDTO) {
        ...
        sendSyncMessage(device.getId(), device.getName(), device.getMax_consumption());
    ...
    public DeviceDetailsDTO update(UUID id, DeviceDetailsDTO deviceDetailsDTO) {
        ...
        sendSyncMessage(updatedDevice.getId(), updatedDevice.getName(), updatedDevice.getMax_consumption());
    ...
    private void sendSyncMessage(UUID id, String name, int maxConsumption) {
        try {
            DeviceToMonitoringDTO syncDto = new DeviceToMonitoringDTO(id, name, maxConsumption);
            rabbitTemplate.convertAndSend(RabbitMQConfig.DEVICE_SYNC_QUEUE, syncDto);
            LOGGER.info("Sent sync message to Monitoring: {}", syncDto);
        } catch (Exception e) {
