...
import { Container, Card, CardBody, Col, Row, Input, Button } from 'reactstrap';
import * as API_CLIENT from "./api/client-api";
import ChatWindow from '../chat/ChatWindow';

const ADMIN_ID = "743559fa-642b-42a6-8e54-d81998a81d24";
...
const switchInputStyle = { opacity: 0, width: 0, height: 0 };
...
class ClientDashboard extends React.Component {
    constructor(props) {
        ...
        this.handleViewStatistics = this.handleViewStatistics.bind(this);
    ...
    handleViewStatistics(deviceId, deviceName, maxConsumption) {
        this.props.history.push({
            pathname: `/device-stats/${deviceId}`,
            state: { deviceName: deviceName, maxConsumption: maxConsumption }
        });
    }
    ...
    render() {
        ...
        const currentUserObj = { id: this.state.currentUserId, name: this.state.username };

        const adminPartner = { id: ADMIN_ID, name: "Support Team (Admin)" };
        ...
        return (
            <div style={backgroundStyle}>
                            ...
                            processedDevices.map(device => (
                                <Col sm="12" md="6" lg="4" key={device.id} className="mb-4">
                                    <Card style={{
                                        ...
                                        border: device.isActive ? 'none' : '1px solid #e2e8f0'
                                    }}>
                                        <CardBody className="d-flex flex-column p-4">
                                            ...
                                            <div className="d-flex align-items-center mb-4 p-3 rounded" style={{ backgroundColor: '#f8fafc' }}>
                                                ...
                                                <span style={{
                                                    fontWeight: '700',
                                                    fontSize: '0.85rem',
                                                    color: device.isActive ? '#16a34a' : '#dc2626',
                                                    letterSpacing: '0.5px'
                                                }}>
                                            ...
                                            <button
                                                ...
                                                onClick={() => this.handleViewStatistics(device.id, device.name, device.max_consumption)}
                ...
                {!this.state.isChatOpen && (
                    <button style={floatingChatButtonStyle} onClick={this.toggleChat} title="Chat with Support">
                        💬
                    </button>
                )}
                ...
                <ChatWindow
                    ...
                    chatPartner={adminPartner}
                    isVisible={this.state.isChatOpen}
